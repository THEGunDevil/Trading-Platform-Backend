...
func LoginHandler(c *gin.Context) {
	...
	accessToken, err := service.GenerateAccessToken(user.ID.String(), user.Role.String, user.TokenVersion, user.IsBanned.Bool, user.IsPermanentBan.Bool)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to generate access token"})
		return
	}

	refreshToken, err := service.GenerateRefreshToken(user.ID.String(), user.TokenVersion, user.IsBanned.Bool, user.IsPermanentBan.Bool)
	if err != nil {
...
func RefreshHandler(c *gin.Context) {
	...
	accessToken, err := service.GenerateAccessToken(userIDStr, user.Role.String, user.TokenVersion, user.IsBanned.Bool, user.IsPermanentBan.Bool)
	if err != nil {
	...
	// Renew refresh token
	refreshToken, err := service.GenerateRefreshToken(userIDStr, user.TokenVersion, user.IsBanned.Bool, user.IsPermanentBan.Bool)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to generate refresh token"})
		return
	}
